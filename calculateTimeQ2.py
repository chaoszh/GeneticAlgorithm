...
def repTime(train):
    ...
    while(wait or aWork or bWork or cWork):
        if(train):
        ...
        for i in range(len(bWork)):
            ...
            if(bWork[i].repairTime >= bWork[i].bTime):
                if(cWork[bWork[i].cWork].level == 0):
        ...
        for i in range(len(aWork)):
            ...
            if(aWork[i].repairTime >= aWork[i].aTime):
                if(bWork[aWork[i].bWork].level == 0):
...
#     '/Users/return0/Docarwnloads/QQFileRecv/20190429-204343generatetable.xlsx')
...
while True:
    ...
    filepath=mt(filepath,index,carnum=carnum,boxNum=boxnum,d=0,e=0)
